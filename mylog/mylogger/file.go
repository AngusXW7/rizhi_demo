...
func (f *FileLogger) initFile() error {
	...
	if err != nil {
		fmt.Printf("open log file failes ,err:%v\n", err)
		return err
	}
	errfileObj, err := os.OpenFile(fullFileName+".err", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		fmt.Printf("open err log file failes ,err:%v\n", err)
		return err
	}
	//日志文件都已经打开了
	f.fileObj = fileObj
...
func (f *FileLogger) spiltFile(file *os.File) (*os.File, error) {
	...
	newLogName := fmt.Sprintf("%s.bak%s", logName, nowStr) //拼接一个日志文件的备份名字
	//1.关闭当前日志文件
	file.Close()
	//2.rename(备份一下)
...
func (f *FileLogger) log(lv LogLevel, format string, a ...interface{}) {
	if f.enable(lv) {
		...
		if lv >= ERROR {
			//如果要记录的日志大于等于ERROR级别，我还要在err日志中在记录一遍
